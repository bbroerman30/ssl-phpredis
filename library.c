        ...
        int str_offset = 0;
	      ...
	      if( use_ssl == 1 ) {
            ...
            if( strlen(INI_STR( "redis.ssl_cafile" )) > 0 ) {
                zval cn_name_zval;
                ZVAL_STRING(&cn_name_zval,INI_STR( "redis.ssl_cafile" ), 0);
                php_stream_context_set_option(context, "ssl", "cafile",&cn_name_zval);
            }   
        
            if( strlen(INI_STR( "redis.ssl_capath" )) > 0 ) {
                zval cn_name_zval;
                ZVAL_STRING(&cn_name_zval,INI_STR( "redis.ssl_capath" ), 0);
                php_stream_context_set_option(context, "ssl", "capath",&cn_name_zval);
            }   

            if( strlen(INI_STR( "redis.ssl_ciphers" )) > 0 ) {
                zval cn_name_zval;
                ZVAL_STRING(&cn_name_zval,INI_STR( "redis.ssl_ciphers" ), 0);
                php_stream_context_set_option(context, "ssl", "ciphers",&cn_name_zval);
            }   

            if( INI_INT("redis.ssl_allow_self_signed") == 1 ) {
                zval allow_self_signed_zval;
                ZVAL_TRUE(&allow_self_signed_zval); 
                php_stream_context_set_option(context, "ssl", "allow_self_signed",&allow_self_signed_zval);
            }
	           
	          if( INI_INT("redis.ssl_disable_compression") == 1 ) {
                zval disable_compression_zval;
                ZVAL_TRUE(&disable_compression_zval);
