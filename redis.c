...
PHP_INI_BEGIN()
	...
	PHP_INI_ENTRY("redis.ssl_allow_self_signed", "0", PHP_INI_ALL, NULL)
	PHP_INI_ENTRY("redis.ssl_cafile", "", PHP_INI_ALL, NULL)
	PHP_INI_ENTRY("redis.ssl_capath", "", PHP_INI_ALL, NULL)
	PHP_INI_ENTRY("redis.ssl_verify_depth", "", PHP_INI_ALL, NULL)
	PHP_INI_ENTRY("redis.ssl_ciphers", "", PHP_INI_ALL, NULL)
	PHP_INI_ENTRY("redis.ssl_disable_compression", "0", PHP_INI_ALL, NULL)
...
PHP_MINFO_FUNCTION(redis)
{
    ...
    if(INI_INT("redis.use_ssl") == 1) {
        ...
        if( strlen(INI_STR( "redis.ssl_cafile" )) > 0 ) {
            php_info_print_table_row(2, "SSL CA File", ( (strlen(INI_STR("ssl_cafile")) > 0) ? INI_STR("ssl_cafile") : "") );
        }
        
        if( strlen(INI_STR( "redis.ssl_cafile" )) > 0 ) {
            php_info_print_table_row(2, "SSL CA Path", ( (strlen(INI_STR("ssl_capath")) > 0) ? INI_STR("ssl_capath") : "") );
        }        
        
        if( strlen(INI_STR( "redis.ssl_ciphers" )) > 0 ) {
            php_info_print_table_row(2, "SSL Ciphers", ( (strlen(INI_STR("ssl_ciphers")) > 0) ? INI_STR("ssl_ciphers") : "") );
        }        
        
        if(INI_INT("redis.ssl_allow_self_signed") == 1) {
            php_info_print_table_row(2, "Allow self-signed certificates", ( (strlen(INI_STR("redis.ssl_allow_self_signed")) > 0) ? INI_STR("redis.ssl_allow_self_signed") : "") );
        }

        if(INI_INT("redis.ssl_disable_compression") == 1) {
            php_info_print_table_row(2, "Disable SSL Compression", ( (strlen(INI_STR("redis.ssl_disable_compression")) > 0) ? INI_STR("redis.ssl_disable_compression") : "") );
        }
        
        if( strlen(INI_STR( "redis.ssl_verify_depth" )) > 0 ) {
            php_info_print_table_row(2, "SSL Verify Depth", ( (strlen(INI_STR("ssl_verify_depth")) > 0) ? INI_STR("ssl_verify_depth") : "") );        
        }
    }
